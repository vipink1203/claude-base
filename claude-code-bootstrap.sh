...
# Scaffolds Claude/Gemini/Codex project assistant workflows into any project
...
ENABLE_EXPERIMENTAL_SUBAGENTS=false
TARGET_CLAUDE=false
TARGET_GEMINI=false
TARGET_CODEX=false
...
Scaffold Claude, Gemini, and Codex assistant workflows into your project.
...
${BOLD}Options:${NC}
  ...
  --agent-platform LIST  Agent platform(s): claude, gemini, codex, auto (default: claude)
  --enable-experimental-subagents
                         Enable Gemini subagent scaffolding (experimental)
...
${BOLD}Examples:${NC}
  ...
  $0 --agent-platform codex       # Bootstrap Codex-specific files
  $0 --agent-platform claude,codex ./my-app
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --enable-experimental-subagents) ENABLE_EXPERIMENTAL_SUBAGENTS=true; shift ;;
...
resolve_agent_platforms() {
    ...
    if [[ "$normalized" == "auto" ]]; then
        [[ -d "$PROJECT_DIR/.claude" || -f "$PROJECT_DIR/CLAUDE.md" ]] && TARGET_CLAUDE=true
        [[ -d "$PROJECT_DIR/.gemini" || -f "$PROJECT_DIR/GEMINI.md" ]] && TARGET_GEMINI=true
        [[ -f "$PROJECT_DIR/AGENTS.md" || -d "$PROJECT_DIR/.agents/skills" ]] && TARGET_CODEX=true

        if ! $TARGET_CLAUDE && ! $TARGET_GEMINI && ! $TARGET_CODEX; then
            TARGET_CLAUDE=true
        fi
        return
    fi
    ...
    for platform in "${selected[@]}"; do
        ...
        case "$platform" in
            ...
            gemini) TARGET_GEMINI=true ;;
            codex) TARGET_CODEX=true ;;
            "") ;;
            *)
                log_error "Invalid agent platform: $platform (must be claude, gemini, codex, or auto)"
                exit 1
                ;;
        esac
    done
    ...
    if ! $TARGET_CLAUDE && ! $TARGET_GEMINI && ! $TARGET_CODEX; then
        log_error "No valid agent platforms selected. Use --agent-platform claude|gemini|codex|auto"
        exit 1
    fi
}

selected_platforms_string() {
    local names=""
    $TARGET_CLAUDE && names+="claude, "
    $TARGET_GEMINI && names+="gemini, "
    $TARGET_CODEX && names+="codex, "
    echo "${names%, }"
}

resolve_agent_platforms "$AGENT_PLATFORM"
SELECTED_AGENT_PLATFORMS="$(selected_platforms_string)"
if [[ "$(echo "$AGENT_PLATFORM" | tr '[:upper:]' '[:lower:]')" == "auto" ]]; then
    log_info "Auto-detected agent platform(s): ${BOLD}$SELECTED_AGENT_PLATFORMS${NC}"
fi
...
echo ""
echo -e "${BOLD}${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║       Agent Platform Bootstrap v${VERSION}                    ║${NC}"
echo -e "${BOLD}${CYAN}║       Multi-Agent Platform Scaffold                       ║${NC}"
echo -e "${BOLD}${CYAN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
log_info "Project directory: ${BOLD}$PROJECT_DIR${NC}"
log_info "Project name:      ${BOLD}$PROJECT_NAME${NC}"
log_info "Stack:             ${BOLD}$STACK${NC}"
log_info "Agent platform(s): ${BOLD}$SELECTED_AGENT_PLATFORMS${NC}"
if $HAS_BE; then
    log_info "Database:          ${BOLD}$DB_NAME${NC} (port $DB_PORT)"
fi
if $TARGET_GEMINI; then
    log_info "Gemini subagents:  ${BOLD}$ENABLE_EXPERIMENTAL_SUBAGENTS${NC} (experimental)"
fi
...
if $UNINSTALL; then
    echo ""
    log_step "Uninstalling bootstrap scaffold"
    ...
    BOOTSTRAP_FILES=(
        ...
        ".gemini/commands/project-help.toml"
        ".gemini/agents/README.md"
        ".agents/skills/project-help/SKILL.md"
        "docs/help/claude.md"
        "docs/help/gemini.md"
        "docs/help/codex.md"
        "docs/README.md"
    ...
    case "$confirm" in
        [Yy]*)
            ...
            # Use find to recursively remove all empty dirs under generated roots.
            for top in ".claude" ".gemini" ".agents" "docs"; do
            ...
            if [[ -d "$PROJECT_DIR/.gemini" ]]; then
                echo ""
                log_warn "Directory ${BOLD}.gemini${NC} was not removed because it contains non-bootstrap files:"
                (cd "$PROJECT_DIR" && find .gemini -type f | sed 's/^/    /')
                echo ""
                echo -e "  To remove everything, run: ${BOLD}rm -rf $PROJECT_DIR/.gemini${NC}"
            fi

            if [[ -d "$PROJECT_DIR/.agents" ]]; then
                echo ""
                log_warn "Directory ${BOLD}.agents${NC} was not removed because it contains non-bootstrap files:"
                (cd "$PROJECT_DIR" && find .agents -type f | sed 's/^/    /')
                echo ""
                echo -e "  To remove everything, run: ${BOLD}rm -rf $PROJECT_DIR/.agents${NC}"
            fi
            ...
            if [[ -f "$PROJECT_DIR/.gitignore" ]]; then
                ...
                sed -i.bak '/# Local AI assistant overrides/d;/\.claude\/settings\.local\.json/d;/CLAUDE\.local\.md/d;/\.gemini\/settings\.local\.json/d;/GEMINI\.local\.md/d;/AGENTS\.local\.md/d' "$PROJECT_DIR/.gitignore"
                rm -f "$PROJECT_DIR/.gitignore.bak"
                log_success "Cleaned local assistant entries from ${CYAN}.gitignore${NC}"
            fi
...
GITIGNORE_ENTRIES=(
    "# Local AI assistant overrides (do not commit personal settings)"
)

$TARGET_CLAUDE && GITIGNORE_ENTRIES+=(".claude/settings.local.json" "CLAUDE.local.md")
$TARGET_GEMINI && GITIGNORE_ENTRIES+=(".gemini/settings.local.json" "GEMINI.local.md")
$TARGET_CODEX && GITIGNORE_ENTRIES+=("AGENTS.local.md")
...
if [[ -f "$PROJECT_DIR/.gitignore" ]]; then
    ...
    for entry in "${GITIGNORE_ENTRIES[@]}"; do
        ...
        if $DRY_RUN; then
            echo -e "  ${DIM}(dry-run)${NC} would append '${CYAN}$entry${NC}' to ${CYAN}.gitignore${NC}"
        else
    ...
    if $DRY_RUN; then
        :
    elif $GITIGNORE_UPDATED; then
        log_success "Updated ${CYAN}.gitignore${NC} with local assistant entries"
    else
        log_info ".gitignore already contains local assistant entries — skipped."
    fi
else
    GITIGNORE_CONTENT=""
    for entry in "${GITIGNORE_ENTRIES[@]}"; do
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║ Gemini platform files                                                     ║
# ╚════════════════════════════════════════════════════════════════════════════╝
if $TARGET_GEMINI; then
    log_step "Gemini platform files"

    GEMINI_CONTENT="# Project Context

This project is bootstrapped for Gemini CLI workflows.

## Stack
- Stack profile: **$STACK**
- Frontend present: **$HAS_FE**
- Backend present: **$HAS_BE**

## Workflow
- Use \`/project-help\` for Gemini-specific commands and conventions.
- Use project commands from \`.gemini/commands/\`.
- Keep implementation aligned with repo docs in \`README.md\` and \`USER_GUIDE.md\`.
"

    if $ENABLE_EXPERIMENTAL_SUBAGENTS; then
        GEMINI_CONTENT+="
## Experimental Features
- Gemini subagent scaffolding is enabled.
- Treat subagents as experimental and validate behavior per release notes."
    fi

    write_file "GEMINI.md" "$GEMINI_CONTENT"

    write_file_heredoc ".gemini/commands/project-help.toml" <<'GEMINI_HELP_CMD'
description = "Show Gemini project help and workflow guidance"

prompt = """
# Gemini Project Help

## Primary Workflow
1. Plan work in small, verifiable increments.
2. Implement changes with tests where possible.
3. Validate lint/tests before shipping.

## Project Files
- `GEMINI.md` contains Gemini-specific project context.
- `README.md` and `USER_GUIDE.md` contain shared repository guidance.

## BMAD Planning
- This repository's BMAD planning agents are Claude-native.
- In Gemini, use equivalent manual planning docs:
  - `docs/briefs/product-brief.md`
  - `docs/prd.md`
  - `docs/architecture.md`
  - `docs/stories/*.md`

## Help
- Gemini command help: `docs/help/gemini.md`
"""
GEMINI_HELP_CMD

    if $ENABLE_EXPERIMENTAL_SUBAGENTS; then
        write_file_heredoc ".gemini/agents/README.md" <<'GEMINI_AGENTS'
# Gemini Subagents (Experimental)

This directory is reserved for Gemini subagent definitions.

Subagent behavior is experimental. Before relying on them in CI or release workflows:
- verify compatibility with your installed Gemini CLI version
- confirm tool access and guardrails explicitly
- keep agent scopes narrow and auditable
GEMINI_AGENTS
    fi

    write_file_heredoc "docs/help/gemini.md" <<'GEMINI_HELP_DOC'
# Gemini Help

Use `/project-help` inside Gemini CLI to load project-specific guidance from `.gemini/commands/project-help.toml`.

Core files:
- `GEMINI.md` — Gemini context
- `README.md` — repository overview
- `USER_GUIDE.md` — workflow details
GEMINI_HELP_DOC
fi
...
# ╚════════════════════════════════════════════════════════════════════════════╝
if $TARGET_CODEX; then
    log_step "Codex platform files"

    CODEX_CONTENT="# AGENTS.md

This repository includes Codex-specific project instructions.

## Quick Start
- Use \`\$project-help\` to load the Codex project-help skill.
- Keep changes small and verifiable.
- Follow stack constraints from \`README.md\` and \`USER_GUIDE.md\`.
...
- \`docs/stories/*.md\`
"
    write_file "AGENTS.md" "$CODEX_CONTENT"

    write_file_heredoc ".agents/skills/project-help/SKILL.md" <<'CODEX_SKILL_HELP'
---
name: project-help
...
CODEX_SKILL_HELP
    ...
    write_file_heredoc "docs/help/codex.md" <<'CODEX_HELP_DOC'
# Codex Help
...
if $TARGET_CLAUDE; then
    write_file_heredoc "docs/help/claude.md" <<'CLAUDE_HELP_DOC'
# Claude Help

Use `/project-help` in Claude Code to show the project command guide from `.claude/commands/project-help.md`.

Core files:
- `CLAUDE.md` — Claude project context
- `.claude/commands/project-help.md` — command help
- `README.md` and `USER_GUIDE.md` — shared workflow docs
CLAUDE_HELP_DOC
fi

if ! $TARGET_CLAUDE && { $TARGET_GEMINI || $TARGET_CODEX; }; then
    log_step "Gitignore updates"
...
echo -e "${BOLD}${GREEN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${BOLD}What was created (stack: $STACK, platform(s): $SELECTED_AGENT_PLATFORMS):${NC}"
echo ""

if $TARGET_CLAUDE; then
echo -e "  ${CYAN}CLAUDE.md${NC}                         Claude project context"
if $HAS_FE; then
...
QUALITY_AGENT_COUNT=0
if [[ "$AGENT_COUNT" -gt 4 ]]; then
QUALITY_AGENT_COUNT=$(( AGENT_COUNT - 4 ))
fi
echo -e "  ${CYAN}.claude/agents/*.md${NC}               $AGENT_COUNT agents (4 BMAD planning + $QUALITY_AGENT_COUNT quality gates)"
echo -e "  ${CYAN}.claude/commands/project-help.md${NC}  Claude /project-help command"
...
if $TARGET_GEMINI; then
echo -e "  ${CYAN}GEMINI.md${NC}                         Gemini project context"
echo -e "  ${CYAN}.gemini/commands/project-help.toml${NC} Gemini /project-help command"
if $ENABLE_EXPERIMENTAL_SUBAGENTS; then
echo -e "  ${CYAN}.gemini/agents/README.md${NC}          Gemini subagent scaffold (experimental)"
fi
fi

if $TARGET_CODEX; then
echo -e "  ${CYAN}AGENTS.md${NC}                         Codex project instructions"
echo -e "  ${CYAN}.agents/skills/project-help/SKILL.md${NC} Codex \$project-help skill"
fi

if $TARGET_CLAUDE; then
echo -e "  ${CYAN}docs/help/claude.md${NC}               Claude help reference"
fi
if $TARGET_GEMINI; then
echo -e "  ${CYAN}docs/help/gemini.md${NC}               Gemini help reference"
fi
if $TARGET_CODEX; then
echo -e "  ${CYAN}docs/help/codex.md${NC}                Codex help reference"
fi
...
if $TARGET_CLAUDE; then
echo ""
echo -e "${BOLD}How Claude hooks work:${NC}"
echo ""
...
if $TARGET_CLAUDE; then
echo -e "  2. Review & customize ${CYAN}CLAUDE.md${NC} for your specific Claude workflow"
echo -e "  3. In Claude, run ${CYAN}/project-help${NC} for command and BMAD guidance"
echo -e "  4. Start with ${CYAN}claude --agent analyst${NC} for planning (BMAD)"
fi
if $TARGET_GEMINI; then
echo -e "  5. In Gemini, run ${CYAN}/project-help${NC} to load Gemini-specific help"
fi
if $TARGET_CODEX; then
echo -e "  6. In Codex, invoke ${CYAN}\$project-help${NC} to load the Codex help skill"
fi
echo ""
